...
export class QQMessageAdapter {
  ...
  static logMessageInfo(qqMsg: QQMessage): void {
    ...
    if (qqMsg.message_type === 'group') {
      logger.info(`📨 收到${messageType}消息 [${groupInfo}] ${userDisplayName}(${qqMsg.user_id}): ${content}`, detailInfo);
    } else {
      logger.info(`💬 收到${messageType}消息 ${userDisplayName}(${qqMsg.user_id}): ${content}`, detailInfo);
    }
  }
